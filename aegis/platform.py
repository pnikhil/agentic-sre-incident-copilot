...
A profile maps abstract providers to concrete adapters. For now, local-fixtures
is wired and gcp-cloud-run is declarative for the later GCP reference deployment.
The core stays platform-agnostic.
"""
...
def check_capabilities(profile: Profile, required: list[Capability]) -> None:
    ...
    Run before adapter wiring, so a wrong profile fails early.
    """
    ...
    if missing:
        raise ValueError(
            ...
            f"{[c.value for c in missing]}. Pick a profile that supports them."
        )
...
def _not_implemented(provider: str, kind: str) -> None:
    raise NotImplementedError(
        f"The '{provider}' {kind} provider is not implemented yet. For now only "
...
def build_adapters(profile: Profile, *, data_dir: Path, artifacts_dir: Path) -> Adapters:
    ...
    Only local providers are implemented now. Other providers raise a clear
    NotImplementedError, so the seam stays honest.
    """
